...
watch(() => props.activeRoute, (newValue) => {
...
<template>
  <div>
            ...
            <div class="space-y-1">
              <div v-for="item in navItems" :key="item.route || item.label">
                ...
                <div
                  ...
                  @click.prevent="item.children && item.children.length > 0
                    ? toggleItemExpansion(item.label)
                    : !item.disabled && handleNavigate(item)"
                >
                  <div class="flex items-center">
                    <component v-if="item.icon" :is="item.icon" class="mr-3 flex-shrink-0 h-6 w-6" />
                    {{ item.label }}
                  </div>
                  <component
                ...
                <div
                  ...
                  :class="[
                    'mt-1 space-y-1 pl-4',
                    expandedItems[item.label] ? 'block' : 'hidden'
                  ]"
                >
                  <a
                  ...
                  >
                    <component v-if="child.icon" :is="child.icon" class="mr-3 flex-shrink-0 h-5 w-5" />
                    {{ child.label }}
                  </a>
                </div>
              </div>
    ...
    <div class="hidden md:flex md:flex-col md:fixed md:inset-y-0" :style="{ width: isOpen ? props.width : 'auto' }">
          ...
          <nav class="flex-1 px-4 py-4">
            <div class="space-y-1">
              <div v-for="item in navItems" :key="item.route || item.label">
                ...
                >
                  <div class="flex items-center">
                    <component v-if="item.icon" :is="item.icon" class="mr-3 flex-shrink-0 h-5 w-5" />
                    {{ item.label }}
                  </div>
                  <component
                    v-if="item.children && item.children.length > 0"
                ...
                >
                  <a
                    ...
                    @click.prevent="!child.disabled && handleNavigate(child)"
                  >
                    <component v-if="child.icon" :is="child.icon" class="mr-3 flex-shrink-0 h-4 w-4" />
                    <span class="flex items-center">
                      <component v-if="child.children && child.children.length > 0" :is="ChevronRight" class="mr-1 h-3 w-3" />
                      {{ child.label }}
                    </span>
                  </a>
                </div>
